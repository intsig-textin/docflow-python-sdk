...
- ✅ **字段管理**：类别字段的增删改查及配置管理，支持批量新增/更新
- ✅ **表格管理**：类别表格的增删改查，支持批量新增/更新（含内嵌字段）
- ✅ **样本管理**：样本文件的上传、下载、删除，支持批量上传/批量下载（ZIP）
...
- ✅ **智能重试**：基于 urllib3.util.retry.Retry 的灵活重试机制
  ...
  - 可配置重试方法（默认：GET, POST, PUT, DELETE, PATCH）
...
> 💡 **想快速体验完整流程？** 查看 [费用报销场景快速开始指南](examples/QUICKSTART.md)，5分钟完成从创建空间到智能审核的端到端流程！
...
print(f"权限范围: {detail.auth_scope}")
print(f"管理员ID: {detail.manage_account_id}")
```
...
result = client.category.fields.batch_add(
    ...
    with_detail=True,  # 返回完整字段详情
)
for f in result.fields:
    print(f"{f.field_id}: {f.name}")
...
# 工作空间操作
ws.get()                      # 获取详情
ws.update(name="新名称", auth_scope=AuthScope.PUBLIC)  # 更新（name 和 auth_scope 都是必填）
ws.delete()                   # 删除
